...
import 'package:extra_pos/modules/lookup/presentation/tolerance_code_lookup/tolerance_code_lookup_controller.dart';
import 'package:flutter/material.dart';
import 'package:get/get.dart';


class ToleranceCodeLookupPage extends StatelessWidget {
  final Function(ToleranceCode selectedTOleranceCode)? onToleranceCodeSelected;

  ToleranceCodeLookupPage({this.onToleranceCodeSelected});

  var controller;

  @override
  Widget build(BuildContext context) {
    controller = Get.put(ToleranceCodeLookupController());
    controller.resetState();

    return BaseScaffold(

        shouldIncludeScrolling: false,
        appBar: BaseAppBar(title: 'Select Tolerance Code'),
        body: Column(
  ...
  _buildSearchFiltersUi() {
    return SearchExpandableCard(
      ...
      body: Column(
        children: [
          AppTextInputField(
              title: 'Search by Tolerance Code',
              hint: 'Search',
              controller: controller.searchByToleranceCodeTextController,
              suffixIcon: Icon(Icons.search),
              onTap: () {
                controller
                    .searchInputFocused(ToleranceCodeSearchFieldType.SEARCH_BY_CODE);
              }
          ),
          AppTextInputField(
              title: 'Search by Description',
              hint: 'Search',
              controller: controller.searchByToleranceDescriptionTextController,
              suffixIcon: Icon(Icons.search),
              onTap: () {
                controller.searchInputFocused(
                    ToleranceCodeSearchFieldType.SEARCH_BY_DESCRIPTION);
              }
          )
        ],
      ),
      ...
      return Expanded(
        child: AppPaginationView(
          ...
          onListReady: (int start, int limit) {
            controller.getToleranceCodes(start: start, limit: limit);
          },
          ...
          itemBuilder: (BuildContext context, int index) {
            return ListItemCard(
              onTap: () {
                Get.back();
                onToleranceCodeSelected!(controller.state.response[index]);
              },
              isMoreButtonNeeded: false,
              title: 'Code',
              titleValue: controller.state.response[index].toleranceCode,
              subTitle: 'Desc.',
              subTitleValue: controller.state.response[index].description,
            );
          },
        ),
      );
